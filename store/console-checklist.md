...
- [ ] Test on physical Android and iOS devices: consent v5, typed and completed voice history across End/leave/relaunch, confirmed Clear chat with the welcome retained, AI Listen playback with no device-reader fallback, confirmation that tapping the voice orb requests microphone permission and opens a WebRTC media stream, microphone transmission from the first orb tap through the second send tap, the disabled track between turns, stream release at End/leave/background, confirmation that live voice does not create a recording file or capture microphone audio in the background, continuous in-memory Hindi-to-English translation start/stop with no recording file, pronunciation cancellation on background and the 15-second stop, English/Hindi reply selection, reporting, consent withdrawal, airplane mode, timeout, and relaunch.
