...
- [ ] Test on physical Android and iOS devices: consent v9, typed and completed voice history across End/leave/relaunch, confirmed Clear chat with the welcome retained, AI Listen playback with no device-reader fallback, confirmation that tapping the voice orb requests microphone permission and opens a WebRTC media stream, your microphone stays on continuously until you mute it, unmuting while Asha speaks, stream release at End/leave/background, confirmation that live voice does not create a recording file or capture microphone audio in the background, pronunciation cancellation on background and the 15-second stop, English/Hindi reply selection, reporting, consent withdrawal, airplane mode, timeout, and relaunch.
- [ ] Deploy and verify `/api/live-call` and `/api/live-status` for `gpt-live-1`. Keep the standard OpenAI key on the backend, validate/rate-limit SDP session creation, and verify real native audio before release.
