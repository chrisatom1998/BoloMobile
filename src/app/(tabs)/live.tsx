...
export default function LiveScreen() {
  ...
  // On compact iPhones, a horizontal action column leaves too little space for
  // the identity copy. Stack the controls below the copy instead.
  const narrowHeaderLayout = windowWidth <= 430;
  const stackedHeaderLayout = largeTextLayout || narrowHeaderLayout;
  const { elapsedSeconds, reset: resetPracticeTimer } = useForegroundTimer();
  const { addPracticeSeconds, aiConsent, appendChatMessages, chatHistory, clearChatHistory, clientId, learnerProfile, markLiveTurn, phraseReviews = {}, phrases = [], togglePhrase, updateLearnerProfile } = useAppState();
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.screen}>
        ...
        ListHeaderComponent={(
          <View>
            <View style={[styles.voiceHero, compactVoiceLayout && styles.voiceHeroCompact, largeTextLayout && styles.voiceHeroLarge, { paddingTop: insets.top + spacing.sm }]} testID="voice-conversation-hero">
              <View style={[styles.topbar, stackedHeaderLayout && styles.topbarStacked, largeTextLayout && { minHeight: Math.ceil(260 * fontScale) }, { width: heroContentWidth }]}>
                <View style={[styles.headerIdentity, stackedHeaderLayout && styles.headerIdentityStacked]}>
                  <View style={[styles.headerCopy, stackedHeaderLayout && styles.headerCopyStacked]}>
                    <JournalKicker>Asha · your practice partner</JournalKicker>
                    <JournalDisplay numberOfLines={stackedHeaderLayout ? undefined : 2} style={[styles.headerTitle, stackedHeaderLayout && styles.headerTitleStacked]}>Let’s begin with a little courage.</JournalDisplay>
                    <Text numberOfLines={stackedHeaderLayout ? undefined : 1} style={[styles.headerSubtitle, stackedHeaderLayout && styles.headerSubtitleStacked]}>Conversational Hindi coach · {responseLanguageName} replies</Text>
                  </View>
                </View>
                <View style={[styles.headerActions, stackedHeaderLayout && styles.headerActionsStacked]}>
                  <View accessibilityLabel="Private conversation" style={styles.privateBadge}>
                    <View style={styles.privateDot} />
                    <Text style={styles.privateText}>Private</Text>
                  </View>
                  <Pressable accessibilityLabel="Go to Today" accessibilityRole="button" onPress={() => router.replace('/')} style={styles.headerButton}>
      ...
      <View style={[styles.composer, { paddingBottom: Math.max(spacing.md, insets.bottom + (Platform.OS === 'ios' ? 52 : spacing.xs)) }]}>
      ...
      {wordDefinitionPhrase ? <WordDefinitionSheet clientId={clientId} onClose={() => setWordDefinitionPhrase(null)} phrase={wordDefinitionPhrase} scriptPreference={learnerProfile.scriptPreference} visible /> : null}
...
export const createLiveStyles = (c: ReturnType<typeof useTheme>['colors']) => ({
  ...
  topbarStacked: { alignItems: 'stretch', flexDirection: 'column', gap: spacing.md, minHeight: 0, paddingRight: 0 },
  headerIdentity: { minWidth: 0, flex: 1, flexDirection: 'row', alignItems: 'flex-start', gap: spacing.sm },
  headerIdentityStacked: { flex: 0 },
  ...
  headerButton: { width: 44, height: 44, borderRadius: radius.pill, borderCurve: 'continuous', borderColor: c.line, borderWidth: StyleSheet.hairlineWidth, backgroundColor: c.paperRaised, alignItems: 'center', justifyContent: 'center' },
